...
function Cart(props) {
    ...
    return <Modal open={props.open} onClose={props.onClose} aria-labelledby="modal-modal-title"
        ...
        <Box className={classes.cart}>
            ...
            <Divider/>
            {products.length === 0 && <Typography variant="body1">Giỏ hàng của bạn đang trống</Typography>}
            {products.length !== 0 && <TableContainer sx={{maxHeight: 300}}>
                <Table stickyHeader aria-label="sticky table">
                    ...
                    <TableBody>
                        {cartCtx.products.map(product => <CartItem
                            onAdd={cartProductAddHandler.bind(null, product.id)}
                            onRemove={cartProductRemoveHandler.bind(null, product.id)} key={product.id}
            ...
            <Stack direction="row" justifyContent="space-between"
                   alignItems="center">
                <Button onClick={props.onClose}><KeyboardArrowLeft/>Tiếp tục mua hàng</Button>
                {products.length !== 0 && <Link to={{
                    pathname: "/order", state: {
                        products: products,
                        total: total
                    }
                }}>
                    <Button onClick={props.onClose} variant="contained">Đặt hàng</Button>
                </Link>}
            </Stack>
