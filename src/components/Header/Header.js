        ...
        <Container>
            <Grid container direction="row" alignItems="center" justifyContent="center">
                <Grid item md={4} xs={12}>
                    <img src="https://bizweb.dktcdn.net/100/446/202/themes/848642/assets/logo.png?1648632451195"
                         alt=""></img>
                </Grid>
                <Grid item md={4} xs={6}>
                    <TextField size="small" fullWidth placeholder="search" variant="outlined"></TextField>
                </Grid>
                <Grid item md={4} xs={6}>
                    <Button className={classes.cartButton} variant="outlined" onClick={props.onShowCart}>
            ...
            <Stack className={classes.routeList} direction="row" spacing={5}>
                ...
                {categories.map(category => <Link key={category.id} style={{textDecoration: "none"}}
                                                  to={{pathname: `/products/${category.name}`, state: category}}>
                    <Typography variant="body1">{category.name.toUpperCase()}</Typography>
                </Link>)}
            </Stack>
        </Container>
        <Divider/>
